...
function avcp_custom_notice()
{
	...
	if ( 'ditte' == $current_screen->taxonomy ) {
		echo '<div class="updated" id="wpgov-message" style="
		font-weight: bold;
		"><p>Attenzione: ogni modifica effettuata a una ditta si applica anche alle gare già pubblicate!<br/>Nel caso di cambio partita iva o denominazione, creare una nuova ditta.</p></div>';
	} else if ( 'areesettori' == $current_screen->taxonomy ) {
		echo '<div class="updated" id="wpgov-message" style="
		font-weight: bold;
		"><p>Attenzione: ogni modifica effettuata a un settore-centro di costo si applica anche alle gare già pubblicate!</p></div>';
	} else if ( 'post' == $current_screen->base && 'avcp' == $current_screen->post_type  ) {
		echo '<noscript><div class="error" style="
		font-weight: bold;
		"><p><center><h3>!!! ATTENZIONE !!!<br/><small>JAVASCRIPT NON ABILITATO</small></h3><br/><br/>IL CORRETTO FUNZIONAMENTO DI QUESTA SCHERMATA E\' VINCOLATO DALL\'ESECUZIONE DI ALCUNI SCRIPT DA PARTE DEL TUO BROWSER<br/>
...
function avcp_admin_messages() {
	...
	if ($get_avcp_invalid == '1') {
		echo '<div class="error"><h4>AVCP</h4><p>Alcuni dataset .xml non hanno superato il controllo di validazione</p><p><a href="' . admin_url() . 'edit.php?post_type=avcp&page=avcp_v_dataset">Clicca qui vedere i dettagli degli errori</a></div>';
	}
	
	if (get_option('avcp_denominazione_ente')  == null || get_option('avcp_codicefiscale_ente') == null) {
		echo '<div class="error"><h4>AVCP</h4><p>Alcuni dati dell\'ente non sono stati specificati in <strong>WPGov.it >> Avcp</strong></b></a></div>';
	}
}
